...
- Barrage-aware Shadowbite: Shadowbite replaces Refulgent Arrow at two nearby targets on an ordinary Hawk's Eye proc, but needs three while Barrage is active, because Barrage makes Refulgent strike three times (840 potency) and only raises Shadowbite to 300 per target. Both thresholds are sliders.
- Multi-dotting: Stormbite and Caustic Bite can be kept on up to three additional engaged enemies that are healthy enough to pay back the GCD, without changing your current target. **Off by default since 0.5.2**: eligibility is still an HP floor rather than a per-target time-to-kill estimate, so on ordinary trash packs it can spend GCDs dotting adds that die first. One checkbox turns it on for durable boss adds.
...
- With **Multi-dot nearby enemies** switched on (it is off by default) and a second engaged dummy in range, both DoTs are applied to it while your target stays unchanged.
...
- The preset buttons restore every default (except the master switch, window visibility, the dismissed notice, and your calibrated gauge indexes) before applying their overrides.
...
- With Barrage active, two nearby enemies produce Refulgent Arrow (three hits) and three produce Shadowbite.
- No action is requested twice in a row while the client still reports it ready: the engine holds an accepted request for 350 ms unless a cast is observed or the action goes on cooldown first.
...
The suite parses every Lua source file and checks advanced-mode invariants including Apex Off, all songs Off, Iron Jaws Off, no DoTs, Heavy Shot fallback, and GCD-only execution. It also covers per-action AoE thresholds, the Barrage-aware Shadowbite/Refulgent decision (including the fallback Barrage timer), pending-request dedupe, the burst DoT gate, coda tracking and Radiant Finale holds, potion weaving and its off/cooldown/missing cases, and multi-dot target selection. `run_gui_tests.py` covers settings persistence, the preset reset, the multi-dot migration, and the ACR stub under reverse load order.
