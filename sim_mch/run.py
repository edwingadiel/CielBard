"""One Machinist fight: `python -m sim_mch.run --seconds 360 --party-buffs 1.10 --timeline 30`."""
...
from .core import FightConfig, load_tables, parse_value, rolled_dps, simulate, standard_party_buffs
...
def add_fight_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--jitter-ms", type=float, default=0.0, help="uniform extra lock, seeded")
...
def config_from_args(args: argparse.Namespace) -> FightConfig:
    ...
    return FightConfig(
        ...
        ping_ms=args.ping_ms, jitter_ms=args.jitter_ms, seed=args.seed, potions=args.potions,
...
def format_report(result, timeline_s: float = 0.0) -> str:
    ...
    for name, damage in sorted(result.damage_by_action.items(), key=lambda kv: -kv[1]):
        casts = result.casts.get(name, "")
        lines.append(f"{name:28s} {casts!s:>6s} {100 * damage / total:6.2f}%")
    lines.append("")
    utility = {k: v for k, v in result.casts.items() if k not in result.damage_by_action}
    lines.append("other casts: " + ", ".join(f"{k} {v}" for k, v in sorted(utility.items())))
    lines.append("wildfire hits: " + str(result.wildfire_hits))
    lines.append("queens (t, battery): " + ", ".join(f"({t:.1f}, {b})" for t, b in result.queens))
    lines.append("")
    ...
    if timeline_s > 0:
        ...
        for entry in result.log:
            if entry["t"] > timeline_s:
                break
            flags = ("  [Reassembled]" if entry.get("reassembled") else "")
            lines.append(f"{entry['t']:7.2f} {'' if entry['gcd'] else '    '}{entry['name']}"
                         f"  heat={entry['heat']} battery={entry['battery']}{flags}")
    return "\n".join(lines)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--rolled", type=int, default=0, metavar="N",
                        help="also roll crit/direct hit N times and report mean and sd")
    ...
    if args.rolled:
        rolls = [rolled_dps(result, tables, args.seed * 100003 + n) for n in range(args.rolled)]
        mean = sum(rolls) / len(rolls)
        sd = (sum((r - mean) ** 2 for r in rolls) / max(1, len(rolls) - 1)) ** 0.5
        print(f"\nrolled x{len(rolls)}: mean {mean:,.1f}  sd {sd:,.1f}")
    if args.json:
